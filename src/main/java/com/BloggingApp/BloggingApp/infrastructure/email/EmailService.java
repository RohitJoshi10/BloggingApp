...
public class EmailService {
    ...
    @Value("${brevo.api.key}") // Railway Dashboard variables se aayega
    private String apiKey;

    @Value("${spring.mail.username}") // Sender email verification ke liye
    ...
    public void sendHtmlEmail(String to, String subject, String htmlBody) {
        try {
            // 1. Headers Setup (Authentication ke liye)
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            headers.set("api-key", apiKey);

            // 2. Body Payload (Brevo API structure ke hisaab se)
            Map<String, Object> requestBody = new HashMap<>();
            requestBody.put("sender", Map.of("name", "Blogging App", "email", senderEmail));
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                System.out.println("✅ Email sent via API successfully to: " + to);
            }
        } catch (Exception e) {
            System.err.println("❌ Brevo API Error while sending to " + to + ": " + e.getMessage());
        }
    }
